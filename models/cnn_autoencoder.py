...
class CNNAutoencoder:
    ...
    def build_model(params):
        ...
        for filters, kernel_size in zip(encoder_filters, kernel_sizes):
            encoded = Conv2D(filters, kernel_size, activation=params["activation"], padding="same")(encoded)

        encoded = GlobalAveragePooling2D()(encoded)
        encoded = Dense(latent_dim, activation=params["activation"])(encoded)

        # Decoder
        decoded = Dense(input_shape[0] * input_shape[1], activation=params["activation"])(encoded)
